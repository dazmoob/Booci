...
class Article_model extends CI_Model {
	...
	/*
		@ Class 	: article_model
		@ Used For 	:
			> Get article data by additional condition
			> For insert, update and delete article data
		@ Function 	:
		...
		@ Notes 	:
			...
			> There are some level in article access, there are :
				1 	=> 	Super Admin 	(All)
				2	=>	Admin 			(All, exc : admin log, article delete admin)
				3	=>	Editor 			(g,h,i)
				4	=>	Writer 			(g,h,i need editor agreement to publish)
				5	=>	article 			(g,h,i read and comment only)
	*/
	...
	*/
	...
	private function select($param = false) {
		...
		if (!empty($param['select'])) :
			...
			switch ($select) :
				...
				case 'user':
					$this->db->select('*, article.id as id, c.id as c_id, c.username as c_username, u.id as u_id, u.username as u_username, article.created_time as created_time, article.updated_time as updated_time, u.created_time as u_created_time, u.updated_time as u_updated_time, c.created_time as c_created_time, c.updated_time as c_updated_time, article.state as state, c.state as c_state, u.state as u_state');
					break;
	...
	private function join($param = false) {
		...
		if (!empty($param['join'])) :
			...
			switch ($param['join']) :
				case 'all':
					// $this->db->join('category', 'article.id_category = category.id');
					break;
				...
				case 'user' :
					$this->db->join('user c', 'article.created_by = c.id');
					$this->db->join('user u', 'article.updated_by = u.id');
					break;
	...
	public function insert($param = false) {
		$this->db->insert('article', $this->set_post());
		return $this->db->insert_id();
	}

	public function update($param = false) {
		$this->condition($param);
		return $this->db->update('article', $this->set_post($param));
	}
	...
	/* 
		...
		check_article, check_slug
	*/

	public function check_article($param = false) {
